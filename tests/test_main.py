import pytest
import requests
from bluebrainatlas_datafetch.main import (randomString, extractListIndexFromPropName, 
                                           createRestFirstSequence, translateFilters,
                                           buildSparqlQuery, getFilteredIds, parse_args, main)
...
def test_buildSparqlQuery():
    ...
    assert (result == 
    "PREFIX unknown: <staging/resources/bbp/atlas/_/>\n" \
...
def test_translateFilters():
    ...
    list_of_args = ['--nexus-token-file', 'tests/empty_token.txt', '--nexus-env', 'env',
                    '--nexus-org', 'org', '--nexus-proj', 'proj', '--out', 'tests/',
                    '--filter', 'property.name="data"', 'resolution.value~=25', 
                    'mba:region.number>=1', 'worldMatrix[0]<10', 'wrong.prop!"wrong"']
    ...
    filters_dict = [dict(zip(filter_properties, filter_1)), 
